...
import { handleLocalFileUploads, generateFileUrl, deleteFile } from "../../middleware/file/multer.middleware.js";
...
const formatCompanyResponse = (company, req, includeRelations = false) => {
    const formatted = {
        ...
        social_media_links: company.social_media_links ? JSON.parse(company.social_media_links) : null,
        documents_url: company.documents_url ? JSON.parse(company.documents_url).map(doc => generateFileUrl(req, doc)) : null,
        is_blocked: company.is_blocked,
        is_premium: company.is_premium,
        is_verified: company.is_verified,
        verified_at: company.verified_at,
        average_rating: company.average_rating ? parseFloat(company.average_rating) : null,
        ...
        verification_status_id: company.verification_status_id ? encodeId(company.verification_status_id) : null,
...
export const createCompany = async (req, res) => {
    try {
        ...
        const finalDocumentPaths = documentPaths.length > 0 ? documentPaths : (uploadedFiles.documents_url ? [uploadedFiles.documents_url] : []);

        // Parse social media links
        let parsedSocialMediaLinks = null;
        if (social_media_links) {
            try {
                parsedSocialMediaLinks = typeof social_media_links === 'string' 
                    ? social_media_links 
        ...
        const company = await prisma.$transaction(async (tx) => {
            ...
            const newCompany = await tx.companies.create({
                data: {
                    ...
                    documents_url: finalDocumentPaths.length > 0 ? JSON.stringify(finalDocumentPaths) : null,
...
export const checkSlugUniqueness = async (req, res) => {
    try {
        ...
        const decodedCompanyId = encodedCompanyId ? decodeId(encodedCompanyId) : null;
...
export const getPremiumCompanies = async (req, res) => {
    try {
        ...
        res.json({
            ...
            data: companies.map(company => formatCompanyResponse(company, req, true)),
            meta: {
                total,
                page: parseInt(page),
...
export const updateCompany = async (req, res) => {
    try {
        ...
        if (name && name !== existingCompany.name) {
            const slug = req.body.slug || (await generateSlug(name, prisma, decodedId));
            const existingSlug = await prisma.companies.findFirst({
                where: {
                    slug,
                    NOT: { id: decodedId },
        ...
        const updateData = {
            ...
            ...(parsedSocialMediaLinks !== undefined && { social_media_links: parsedSocialMediaLinks }),
        ...
        if (newLogoPath) {
            if (existingCompany.logo_url) {
                try {
                    ...
                    console.error(`Error deleting old logo: ${existingCompany.logo_url}`, error);
                }
            }
        ...
        if (name) {
            const slugToUse = req.body.slug || (await generateSlug(name, prisma, decodedId));
